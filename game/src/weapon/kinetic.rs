...
impl KineticGun {
    ...
    fn try_pick_target(&self, graph: &mut Graph) -> Result<Target, Handle<Node>> {
        ...
        if let Some(intersection) = query_buffer.first() {
            let collider = graph.try_get(intersection.collider).unwrap();
...
impl ScriptTrait for KineticGun {
    ...
    fn on_update(&mut self, ctx: &mut ScriptContext) -> GameResult {
        ...
        if self.is_active {
            ...
            if let Some(target) = self.target.as_ref() {
                ...
                if collider.is_globally_enabled() {
                    ...
                    let ray = ctx.scene.graph.try_get_mut(*self.ray)?;
        ...
        ctx.scene
            .graph
            .try_get_mut(*self.ray)?
            .set_visibility(self.target.is_some());

        ctx.scene
            .graph
            .try_get_mut(*self.laser_sight)?
            .set_visibility(self.target.is_none());
